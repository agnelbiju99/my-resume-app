    ...
    <div style={{ fontFamily: "'Times New Roman', Times, serif", fontSize: 10, color: "#111", lineHeight: 1.5 }}>
      ...
      <div style={{ fontSize: 9, color: "#444", borderBottom: "1px solid #888", paddingBottom: 5, marginBottom: 8 }}>
        {[
          ...
          data.phone,
          data.location,
          data.linkedin ? (data.linkedin.match(/linkedin\.com\/in\/([^/]+)/i)?.[1] || data.linkedin).replace(/-?\d+$/, "").replace(/-/g, " ").replace(/\b\w/g, c => c.toUpperCase()) : "",
          data.website ? (data.website.match(/github\.com\/([^/]+)/i)?.[1] || data.website) : ""
        ].filter(Boolean).join("  |  ")}
      </div>
      ...
      {data.experience.some(e => e.title) && <>
        ...
        {data.experience.filter(e => e.title).map((e, i) => (
          <div key={i} style={{ marginBottom: 6 }}>
            ...
            {e.bullets.filter(b => b).map((b, j) => <div key={j} style={{ fontSize: 9, paddingLeft: 10, position: "relative" }}><span style={{ position: "absolute", left: 0 }}>–</span>{b.replace(/^[-•]\s*/, "")}</div>)}
          </div>
        ))}
      </>}
...
export default function ATSResumeGenerator() {
  ...
  data.website ? `<a href="${data.website.startsWith("http")?data.website:"https://"+data.website}" target="_blank" style="color:#333;text-decoration:none"><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"11\" height=\"11\" viewBox=\"0 0 24 24\" style=\"vertical-align:middle;margin-right:3px\"><path fill=\"#333\" d=\"M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.929.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z\"/></svg>${getGitHubDisplay(data.website)}</a>` : ""
          ...
          ${allSkills.length ? `<div style="font-size:10pt;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:${secColor};border-bottom:1px solid #ccc;padding-bottom:2px;margin-bottom:6px;margin-top:12px">Skills</div><div style="font-size:9.5pt;display:flex;flex-wrap:wrap;gap:4px">${allSkills.map(s=>`<span style="background:#f0f0f0;padding:2px 6px;border-radius:3px">${s}</span>`).join(" ")}</div>` : ""}
